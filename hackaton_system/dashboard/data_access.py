"""Загрузчики данных и вспомогательные функции для Streamlit-дашборда."""
...
class DashboardData:
    """Набор датафреймов, который нужен для построения основных графиков."""
...
def _load_activity_df(video_id: Optional[int]) -> DataFrame:
    """Возвращает активности для видео или заглушку, если данных нет."""
    if video_id is None:
...
def _load_headcount_df(video_id: Optional[int]) -> DataFrame:
    """Строит поминутное количество людей на кадре для выбранного видео."""
    if video_id is None:
...
def _load_person_activity_matrix(video_id: Optional[int]) -> DataFrame:
    """Считает длительность активностей в разрезе ролей."""
    if video_id is None:
...
def _fetch_rows(statement: Select[Any]) -> list[dict[str, Any]]:
    """Вспомогательный слой: исполняет запрос и приводит записи к словарям."""
...
def _placeholder_activity_df() -> DataFrame:
    """Заглушка с фиксированными активностями для отображения на дашборде."""
...
def _placeholder_headcount_df() -> DataFrame:
    """Заглушка для графика headcount, если реальных данных нет."""
...
def _placeholder_matrix_df() -> DataFrame:
    """Заглушка для матрицы ролей — имитация данных нескольких профессий."""
...
def _placeholder_episodes_df() -> pd.DataFrame:
    """Заглушка с несколькими эпизодами треков."""
...
def load_activity_summary(video_id: Optional[int]) -> DataFrame:
    """Формирует сводку по активностям с длительностью в минутах."""
    df = _load_activity_df(video_id)
    if df.empty:
        return _placeholder_activity_df()
    # Пересчитываем продолжительность из секунд в минуты.
    if "t_start_sec" in df.columns and "t_end_sec" in df.columns:
        ...
        # Группируем по типу активности и суммируем длительность.
        summary = df.groupby("activity_class", as_index=False)["duration_min"].sum()
        return summary
    # Если колонка duration_min уже есть — просто суммируем её значения.
...
def load_headcount(video_id: Optional[int]) -> DataFrame:
    """Возвращает серию «время → количество людей на кадре»."""
    return _load_headcount_df(video_id)


def load_role_activity_matrix(video_id: Optional[int]) -> DataFrame:
    """Строит матрицу «тип сотрудника × активность» в минутах."""
    df = _load_person_activity_matrix(video_id)
    if df.empty:
        return _placeholder_matrix_df()
    # Конвертируем числовые столбцы (активности) из секунд в минуты.
    df = df.copy()
...
def load_person_episodes(video_id: Optional[int]) -> DataFrame:
    """Возвращает эпизоды активностей людей для таблицы UI."""
    if video_id is None:
...
def delete_video_and_related(video_id: int) -> int:
    """Удаляет видео и все связанные записи, возвращает удалённый video_id или 0."""
...
def save_manual_activity(
    ...
    """Создаёт или правит запись активности, добавленную вручную.

    Args:
        video_id: Идентификатор видео.
        person_id: Идентификатор человека.
        track_id: ID трека для связки с детекциями.
        activity_class: Название класса активности.
        start_sec: Время начала в секундах.
        end_sec: Время окончания в секундах.
        reviewer: Кто утвердил изменение.
        notes: Дополнительные комментарии ревьюера.
        activity_id: Если указан — редактируем существующую запись.

    Returns:
        Объект Activity после сохранения.
    """
    init_db()
    with get_session() as session:
        if activity_id is not None:
            ...
            activity.activity_class = activity_class
            activity.t_start_sec = start_sec
            activity.t_end_sec = end_sec
            activity.source = "manual"
            activity.review_status = "approved"
            activity.reviewer = reviewer
            activity.review_notes = notes
        else:
            # Создаём новую активность со 100% доверием.
            activity = Activity(
                video_id=video_id,
                person_id=person_id,
                ...
                t_end_sec=end_sec,
                activity_conf=1.0,  # Ручной ввод считаем максимально достоверным.
                source="manual",
