"""Генерация демонстрационных данных для БД."""
...
def seed_demo_data(force: bool = False) -> int:
    """Заполняет базу данных фиксированными демо-записями для дашборда."""
    ...
    with get_session() as session:
        ...
        if video_count and not force:
            LOGGER.info(
                'Database already contains data. Skipping demo seeding.'
            )
            return 0
        ...
        for track_id, person_type in enumerate(person_types, start=1):
            # Создаём человека и фиксируем его «профессию».
            person = Person(
                video_id=video.id,
                track_id=track_id,
            ...
            while frame <= 300:
                # Имитируем работу трекера: прямоугольник каждые 15 кадров.
