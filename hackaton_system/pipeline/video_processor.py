"""Хайлевел-пайплайн обработки видео: детекция, ролевая классификация, БД."""
...
try:  # torchreid необязателен; при отсутствии используем torchvision.
...
class DetectorProtocol(Protocol):
    """Минимальный интерфейс, который должна поддерживать модель Ultralytics."""

    def track(self, *args: Any, **kwargs: Any) -> Iterable[Any]:
        """Запускает режим track в модели Ultralytics и отдаёт поток результатов."""
        ...
...
class DetectionResult:
    """Результат детекции с информацией о треке."""
    frame_id: int
    time_sec: float
...
@dataclass(slots=True)
class PoseResult:
    """Пакет с позой (скелетом) для конкретного трека."""
    frame_id: int
    time_sec: float
    track_id: int
...
class TrainDetectionResult:
    """Детекция поезда с дополнительными атрибутами номера."""
    frame_id: int
    time_sec: float
    bbox: tuple[int, int, int, int]
    confidence: float
...
class ActivityResult:
    """Сводная запись об активности трека."""
    track_id: int
    activity_class: str
    start_sec: float
    end_sec: float
...
class RoleAssignment:
    """Назначение роли (типа персонала) треку."""
    track_id: int
    person_type: str
    confidence: float
...
@dataclass(slots=True)
class FrameState:
    """Промежуточное состояние трека между кадрами."""
    track_id: int
    label: str
    start_sec: float
    end_sec: float
...
class VideoProcessor:
    """Главный оркестратор пайплайна: детекция → трекинг → БД/рендер."""

    def __init__(self, settings: Settings | None = None) -> None:
        """Создаёт экземпляр процессора и подготавливает ресурсы (БД, кэш)."""
        ...
        # Поддержка VideoLLaMA3 — инициализируем, если выбран соответствующий backend.
        self._videollama3_inferrer: VideoLLaMA3ActionInferrer | None = None
        backend = getattr(self.settings, 'action_backend', 'heuristic')
        if backend == 'videollama3':
            self._videollama3_inferrer = get_shared_videollama3_instance(self.settings)
        self._ensure_cache_dirs()
        # При создании процессора убеждаемся, что схема БД готова принимать данные.
        init_db()

    def _preprocess_frame(self, frame: np.ndarray) -> np.ndarray:
        """Выполняет лёгкую предобработку кадра (шум, контраст, гамма)."""
    ...
    def _ensure_cache_dirs(self) -> None:
        """Создаёт локальные каталоги для весов и зависимостей (OCR, torch)."""
    ...
    def process_video(self, video_path: str | Path) -> int:
        """Обрабатывает видеофайл от детекции до записи результатов в БД."""
        ...
        # Логируем все YOLO-детекции, сгруппированные по классам.
        LOGGER.info('=' * 80)
        LOGGER.info('YOLO DETECTION RESULTS (by class):')
        
        # Собираем детекции по class_id.
        detections_by_class: dict[int, list[DetectionResult]] = defaultdict(list)
        for det in detections:
            class_id = getattr(det, 'class_id', 0)
            detections_by_class[class_id].append(det)
        
        # Выводим статистику по каждому классу.
        for class_id in sorted(detections_by_class.keys()):
            ...
            LOGGER.info('  %s (class_id=%s) detections: %s', class_name, class_id, len(class_dets))
            if class_dets:
                detections_by_track = self._group_detections_by_track(class_dets)
                LOGGER.info('    %s tracks: %s', class_name, len(detections_by_track))
                for track_id, track_dets in sorted(detections_by_track.items()):
                    ...
                    LOGGER.info(
                        '      Track ID %s: %s detections, time: %s, avg confidence: %.3f',
                        track_id,
                        len(track_dets),
                        time_range,
                        avg_conf,
        ...
        # Отдельно логируем поезд, потому что используем TrainDetectionResult.
        LOGGER.info('  Train (class_id=6) detections: %s', len(train_events))
        if train_events:
            train_by_track: dict[int, list[TrainDetectionResult]] = defaultdict(list)
            for train in train_events:
                train_by_track[train.track_id].append(train)
            LOGGER.info('    Train tracks: %s', len(train_by_track))
            for track_id, track_trains in sorted(train_by_track.items()):
                ...
                LOGGER.info(
                    '      Train Track ID %s: %s detections, time: %s, avg confidence: %.3f%s',
                    track_id,
                    len(track_trains),
                    time_range,
                    avg_conf,
        ...
        LOGGER.info('  Pose detections: %s', len(poses))
        LOGGER.info('=' * 80)
        ...
        activities = self._infer_activities(tracks, fps=fps, video_path=video_path)
        
        # Если активирован VideoLLaMA3 для поездов, запускаем инференс.
        train_activities: list[ActivityResult] = []
        if train_events and self._videollama3_inferrer is not None:
            train_activities = self._infer_train_activities(train_events, video_path, fps)
            activities.extend(train_activities)
        ...
        with get_session() as session:
            ...
            action_backend = getattr(self.settings, 'action_backend', 'heuristic')
            source = 'videollama3' if action_backend == 'videollama3' else 'heuristic'
    ...
    def _extract_video_metadata(self, video_path: Path) -> tuple[float, int, float]:
        """Читает fps, количество кадров и длительность ролика."""
    ...
    def _run_detection(
        ...
        """Запускает детекцию, трекинг и (опционально) поиск поездов YOLO."""
        ...
        track_classes = getattr(
            self.settings, "detection_classes", DETECTION_CLASSES_DEFAULT.copy()
        )
        if not track_classes:
            track_classes = DETECTION_CLASSES_DEFAULT.copy()
        if not enable_train:
            track_classes = [cls for cls in track_classes if cls != 6] or [0]
        tracker_conf = (
    ...
    def _run_detection_patch(
        ...
        """Выполняет детекцию по патчам и трекинг ByteTrack через patched_yolo_infer."""
    ...
    def _infer_activities(
        self, tracks: Mapping[int, list[DetectionResult]], fps: float | None = None, video_path: Path | None = None
    ) -> list[ActivityResult]:
        """Определяет активности с помощью эвристик или VideoLLaMA3 (в зависимости от настроек)."""
        
        # Проверяем, включён ли backend VideoLLaMA3.
        backend = getattr(self.settings, 'action_backend', 'heuristic')
        if backend == 'videollama3' and self._videollama3_inferrer is not None and video_path is not None:
            # Используем VideoLLaMA3 для инференса.
            try:
                # Группируем треки по class_id, чтобы поддерживать несколько классов.
                try:
                    from hackaton_system.pipeline.yolo_classes import get_class_name
                except ImportError:
                    # Фолбэк, если модуль yolo_classes недоступен.
                    def get_class_name(class_id: int) -> str:
                        """Возвращает имя класса для VideoLLaMA3."""
                        return 'person' if class_id == 0 else f'class_{class_id}'
                
                tracks_by_class: dict[int, dict[int, list[DetectionResult]]] = defaultdict(dict)
                for track_id, track_dets in tracks.items():
                    if not track_dets:
                        continue
                    # Все детекции в треке имеют одинаковый class_id; если нет — считаем, что это человек.
                    class_id = getattr(track_dets[0], 'class_id', 0)
                    tracks_by_class[class_id][track_id] = track_dets
                
                # Запускаем инференс отдельно для каждого класса.
                activities: list[ActivityResult] = []
                for class_id, class_tracks in tracks_by_class.items():
                    class_name = get_class_name(class_id)
                    # В target_class отправляем человекочитаемое имя.
                    class_activities = self._videollama3_inferrer.infer_actions(
                        video_path, class_tracks, target_class=class_name
                    )
                    activities.extend(class_activities)
                return activities
            except Exception as exc:
                LOGGER.warning("VideoLLaMA3 inference failed, falling back to heuristic: %s", exc)
                # Переходим на эвристики.
        
        # Эвристический метод (исходная логика).
        if not tracks:
            return []
    ...
    def _infer_train_activities(
        self,
        train_events: list[TrainDetectionResult],
        video_path: Path,
        fps: float,
    ) -> list[ActivityResult]:
        """Определяет активности поездов через VideoLLaMA3, если бэкенд активирован."""
        if not train_events or self._videollama3_inferrer is None:
            return []
        
        # Проверяем, включён ли VideoLLaMA3 для поездов.
        backend = getattr(self.settings, 'action_backend', 'heuristic')
        if backend != 'videollama3':
            return []
        
        # Переводим TrainDetectionResult в DetectionResult и группируем по track_id.
        train_tracks: dict[int, list[DetectionResult]] = defaultdict(list)
        for train_det in train_events:
            train_tracks[train_det.track_id].append(
                DetectionResult(
                    frame_id=train_det.frame_id,
                    time_sec=train_det.time_sec,
                    bbox=train_det.bbox,
                    confidence=train_det.confidence,
                    track_id=train_det.track_id,
                    class_id=6,  # Класс COCO №6 — поезд.
                )
            )
        ...
        # Запускаем VideoLLaMA3 для инференса активностей.
        try:
            train_activities = self._videollama3_inferrer.infer_actions(
                video_path, train_tracks, target_class='train'
            )
            return train_activities
        except Exception as exc:
            LOGGER.warning(
                'VideoLLM train activity inference failed: %s', exc, exc_info=True
            )
            return []
    ...
    def _classify_track_states(
        ...
        """Разбивает трек на состояния (label + промежуток времени)."""
        if len(detections) == 0:
            return []
        ...
        # Добавляем хвост к последнему кадру.
    ...
    def _zone_for_point(self, center: tuple[float, float]) -> ZoneDefinition | None:
        """Возвращает зону настроек, в которой расположен центр трека."""
        x, y = center
    ...
    def _activity_label(self, zone: ZoneDefinition | None, speed: float) -> str:
        """Подбирает текстовую метку активности с учётом зоны и скорости."""
        ...
        if category == "station":
            ...
            return "working"
        # Для коридора, зоны визитёров или отсутствия зоны.
        if speed >= move:
            return "walking"
        if speed <= idle:
            return "standing"
    ...
    def _infer_person_roles(
        ...
        """Назначает ролям треки на основе того, где и сколько человек находился."""
    ...
    def _accumulate_role_time(
        ...
        """Считаем длительность пребывания в каждой зоне для одного трека."""
        frame_step = 1.0 / fps if fps else 1.0 / 25.0
    ...
    def _role_name_for_zone(self, zone: ZoneDefinition | None) -> str:
        """Возвращает человекочитаемое имя роли для заданной зоны."""
        if zone is None:
            return "visitor"
    ...
    def _speed(
        ...
        """Скорость перемещения между двумя точками за указанное время."""
        if duration <= 0.0:
            return 0.0
    ...
    def _ensure_person_records(
        ...
        """Создаёт записи Person для всех треков и возвращает карту track_id → Person."""
    ...
    def _stitch_tracks_with_reid(
        ...
        """Сшивает треки, если эмбеддинги показывают, что это один и тот же человек."""
    ...
    def _compute_track_embeddings(
        ...
        """Агрегирует эмбеддинги треков в нормализованный JSON."""
        result: dict[int, str] = {}
        for track_id, dets in tracks.items():
            embeddings = [
                np.array(det.embedding, dtype=np.float32) for det in dets if det.embedding
    ...
    def _ensure_reid_model(self) -> Any | None:
        """Лениво подгружает модель для ReID и возвращает её."""
    ...
    def _extract_embedding(
        ...
        """Получает ReID-эмбеддинг из вырезанной области кадра."""
    ...
    def _ensure_train_reader(self):
        """Инициализирует EasyOCR для распознавания номеров поездов."""
        if easyocr is None:
            return None
    ...
    def _extract_train_number(
        ...
        """Пытается распознать номер поезда в пределах bbox."""
    ...
    def _store_train_events(
        ...
        """Сериализует найденные события поезда в JSON-отчёт."""
