...
def make_batched_images(images) -> list[list[ImageInput]]:
    ...
    if isinstance(images, (list, tuple)):
        ...
        if not all(
            is_valid_video(image) or is_valid_image(image) for image in images
        ):
            raise ValueError(f'Could not make batched images from {images}')
        return images
    if is_valid_video(images) or is_valid_image(images):
        # single image/video
        return [images]
...
def batched_resize(
    ...
    # global max_pixels
    total_tokens = 0
    for (num_frame, height, width), factor in zip(
        image_sizes, factors, strict=False
    ):
        total_tokens += (
    ...
    if total_tokens > max_tokens:
        ...
        tmp_image_sizes = []
        for (_, height, width), factor in zip(
            image_sizes, factors, strict=False
        ):
            h_bar = math.floor(height / beta / factor) * factor
            w_bar = math.floor(width / beta / factor) * factor
...
class Videollama3ImageProcessor(BaseImageProcessor):
    ...
    def preprocess(
        ...
        if isinstance(merge_size, (list, tuple)):
            assert len(merge_size) == len(images), (
        ...
        for image, merge_size, target_size in zip(
            images, merge_sizes, target_sizes, strict=False
        ):
