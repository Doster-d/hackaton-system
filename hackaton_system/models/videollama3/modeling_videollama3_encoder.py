...
class Videollama3VisionEmbeddings(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        return patch_embeds.view(-1, self.embed_dim)
...
class Videollama3VisionTransformerEncoder(nn.Module):
    ...
    def rot_pos_emb(self, grid_sizes, merge_sizes):
        pos_ids = []
        for (t, h, w), merge_size in zip(
            grid_sizes, merge_sizes, strict=False
        ):
...
class Videollama3VisionEncoderModel(PreTrainedModel):
    ...
    def forward(
        self, pixel_values, grid_sizes, merge_sizes=None
    ) -> torch.Tensor:
        ...
        for hidden_states, grid_size, merge_size in zip(
            hidden_states_chunks, grid_sizes, merge_sizes, strict=False
        ):
