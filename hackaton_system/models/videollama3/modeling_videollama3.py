...
try:
    ...
    Videollama3Qwen2Config = configuration_videollama3.Videollama3Qwen2Config
...
class Videollama3MetaForCausalLM(ABC):
    ...
    def _get_valid_visual_tokens(
        ...
        valid_masks = []
        for num_patches, modal in zip(
            batched_num_patches, modals, strict=False
        ):
        ...
        return mm_features[torch.cat(valid_masks)]
    ...
    def _maybe_truncate_visual_tokens(
        ...
        if (
            ...
            or mm_features.shape[0]
            == input_ids.eq(self.config.image_token_index).sum()
        ):
        ...
        seq_end_indices = [
            *seq_end_indices[seq_end_indices > 0].tolist(),
            len(input_ids),
        ]
        ...
        num_visual_tokens = [
            ...
            for start, end in zip(
                seq_start_indices, seq_end_indices, strict=False
            )
        ]

        for n, mask in zip(num_visual_tokens, truncation_mask, strict=False):
            if len(mask) > 0:
                mask[n:] = False
    ...
    def _get_compression_mask(
        ...
        batched_images = pixel_values.split(
        ...
        for images, num_patches, grid_size, merge_size, modal in zip(
            ...
            if modal == 'image' or (modal == 'video' and t == 1):
                compression_masks.append(
                    torch.ones(
                        (num_patches,), dtype=torch.bool, device=images.device
                    )
                )
                ...
                pixel_diff = torch.cat(
                    [
                        torch.full_like(pixel_diff[0:1], threshold + 1),
                        pixel_diff,
                    ],
                    dim=0,
                )
        ...
        if position_ids is not None:
            ...
            position_ids = torch.cat(
                [
                    ...
                    for start, end in zip(pos_start, pos_end, strict=False)
                ]
            )
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        grid_sizes: torch.LongTensor | None = None,
        merge_sizes: torch.LongTensor | None = None,
        ...
        mm_features = self._get_valid_visual_tokens(
        ...
        if self.config.use_token_compression:
            assert B == 1, (
                'Token compression is only supported for batch_size=1'
            )
            mm_features, input_ids, attention_mask, labels, position_ids = (
                self._compress_visual_tokens(
                    compression_mask,
                    mm_features,
                    input_ids,
                    attention_mask,
        ...
        inputs_embeds[image_selected] = (
            inputs_embeds[image_selected] * 0.0 + mm_features
        )
...
class Videollama3Qwen2ForCausalLM(
    Qwen2ForCausalLM, Videollama3MetaForCausalLM
):
    config_class = Videollama3Qwen2Config
