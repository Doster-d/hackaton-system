...
from hackaton_system.utils.ffmpeg_subprocess import (
    run_ffmpeg_rawvideo,
    run_ffprobe,
)
...
try:
    ...
    is_valid_image = image_processing_videollama3.is_valid_image
    is_valid_video = image_processing_videollama3.is_valid_video
...
BatchedImage = list[SingleImage | SingleVideo]
BatchedNamedImage = list[tuple[str, SingleImage | SingleVideo]]
...
def is_named_image(image) -> bool:
    return (
        ...
        and image[0] in ['image', 'video']
        and (is_valid_image(image[1]) or is_valid_video(image[1]))
    )
    ...
    if isinstance(images, (list, tuple)) and all(
        ...
        for image in images:
            if is_valid_video(image):
                ...
                raise ValueError(
                    f'Could not make batched images from {images}'
                )
        return [x[0] for x in batch], [x[1] for x in batch]
    if is_named_image(images):
        # named images
        return [images[0]], [image[1]]
    if is_valid_video(images):
        # single video
        return ['video'], [images]
...
def frame_sample(
    duration, mode='uniform', num_frames=None, vid_fps=None, fps=None
):
    if mode == 'uniform':
...
def load_video_from_ids(
    video_path, s=None, e=None, fps=None, max_frames=128, temporal_factor=1
):
    ...
    f_end = (
        ...
        if e is None
        else min(int(e * vid_fps) - 1, num_frames_of_video - 1)
    )
    ...
    if os.path.isdir(video_path):
        frames = np.array(
            [
                cv2.cvtColor(
                    cv2.imread(
                        os.path.join(video_path, frame_files[frame_idx])
                    ),
                    cv2.COLOR_BGR2RGB,
                )
    ...
    if temporal_factor > 1:
        ...
        [
            timestamps.append(timestamps[-1] + 1 / fps)
...
class ChatTemplateKwargs(TypedDict, total=False):
    ...
    add_system_prompt: bool | None
    add_generation_prompt: bool | None


class Videollama3Qwen2ProcessorKwargs(
    ProcessingKwargs, ChatTemplateKwargs, total=False
):
...
class Videollama3Qwen2Processor(ProcessorMixin):
    ...
    def _infer_generation_prompt(self):
        ...
        instruction = self.apply_chat_template(
    ...
    def _get_downsampled_grid_sizes(self, image_inputs: dict[str, Any]):
        grid_sizes = []
        for grid_size, merge_size in zip(
            image_inputs.get('grid_sizes', []),
    ...
    def _get_visual_seq_len(self, grid_size: torch.Tensor):
        return int(grid_size.prod().item())

    def load_images(
        self, image_path: str | list[str] | Image.Image | list[Image.Image]
    ):
        def load_single_image(image_path):
            if isinstance(image_path, str) and os.path.isfile(image_path):
                ...
                raise ValueError(
                    f'Unsupported image path type: {type(image_path)}'
                )
            return images
    ...
    def load_video(
        ...
        filters: list[str] = []
        if fps is not None:
            filters.append(f'fps=fps={fps}:round=down')
        if new_w != w or new_h != h:
            filters.append(f'scale={new_w}:{new_h}')
        out = run_ffmpeg_rawvideo(
            video_path,
            pix_fmt='rgb24',
        ...
        frames = (
            ...
            .reshape([-1, new_h, new_w, 3])
            .transpose([0, 3, 1, 2])
        )

        if fps is not None:
            timestamps = np.arange(
                start_time, start_time + duration + 1 / fps, 1 / fps
            )[: len(frames)]
        else:
            timestamps = np.linspace(
                start_time, start_time + duration, len(frames)
            )
        ...
        if temporal_factor > 1:
            ...
            timestamps = np.concatenate(
                [
                    timestamps,
                    timestamps[-1:].repeat(pad_length)
    ...
    def _load_multimodal_data(self, conversation: Conversation):
        ...
        for message in conversation:
            ...
            for content in message['content']:
                ...
                if (
                    ...
                    and isinstance(content[content['type']], dict)
                ):
                    ...
                    data_id = json.dumps(serializable_args)
        ...
        for data_id, contents in multimodal_info.items():
            ...
            if data_type == 'image':
                image = self.load_images(contents[0][data_type]['image_path'])[
                    0
                ]
                ...
                start_times = [
                    content['video'].get('start_time', 0.0)
                    for content in contents
                ]
                end_times = [
                ...
                for content, start_time, end_time in zip(
                    contents, start_times, end_times, strict=False
                ):
                    cur_images, cur_timestamps = [], []
                    for image, timestamp in zip(
                        images, timestamps, strict=False
                    ):
        ...
        for message in conversation:
            ...
            for content in message['content']:
                ...
                if content['type'] == 'video':
                    video = content['video']
                    assert is_valid_video(video), (
                        f'Invalid video data: {video}.'
                    )
    ...
    def _process_conversation_with_label(
        ...
        **kwargs,
        ...
        assert kwargs.pop('return_tensors', 'pt') == 'pt', (
            'Only PyTorch tensors are supported when return_labels=True.'
        )
        assert 'add_generation_prompt' not in kwargs, (
            "'add_generation_prompt' argument is not supported when return_labels=True."
        )
        ...
        for message_idx, message in enumerate(conversation):
            ...
            input_ids = self.tokenizer.encode(
                prompt, return_tensors='pt', **output_kwargs['text_kwargs']
            )[0]
            ...
            if message['role'] == 'assistant':
                targets[self.generation_prompt_length : -1] = input_ids[
                    self.generation_prompt_length : -1
                ].clone()
            ...
            if (
                message_idx > 0
                and conversation[message_idx - 1]['role'] == 'stream'
            ):
        ...
        types, counts = torch.unique(
            sample_types[sample_types > -1], return_counts=True
        )

        if len(types) > 0:
            target_num_samples = counts.amin()
            for type_id, type_count in zip(types, counts, strict=False):
                if type_count > target_num_samples:
                    indices = torch.nonzero(sample_types == type_id)[:, 0]
                    random_selector = torch.randperm(indices.size(0))[
                        :-target_num_samples
                    ]
                    text_inputs['labels'][indices[random_selector]] = (
                        IGNORE_INDEX
                    )
                    # sample_types[indices[random_selector]] = -1

        assert len(grid_sizes) == image_idx, (
            'Number of images does not match the number of image tokens in the text.'
        )
    ...
    def _process_conversation_without_label(
        ...
        **kwargs,
        ...
        return self.process_text(
            prompt, image_inputs, **output_kwargs['text_kwargs']
        )

    def _process_conversation(
        self,
        conversation: Conversation,
        images: BatchedImage | BatchedNamedImage | None = None,
        return_labels: bool = False,
        **kwargs: Unpack[Videollama3Qwen2ProcessorKwargs],
    ) -> BatchFeature:
        assert isinstance(conversation, list), (
            'Conversation must be a list of messages.'
        )
    ...
    def _process_plain(
        self,
        text: TextInput | PreTokenizedInput = None,
        images: BatchedImage | BatchedNamedImage | None = None,
        return_labels: bool = False,
        **kwargs: Unpack[Videollama3Qwen2ProcessorKwargs],
    ) -> BatchFeature:
        ...
        text_inputs = self.process_text(
            text, image_inputs, **output_kwargs['text_kwargs']
        )

        return BatchFeature(data={**text_inputs, **image_inputs})

    def process_images(
        self, images: BatchedImage | BatchedNamedImage, **kwargs
    ):
        ...
        **kwargs,
        ...
        if len(grid_sizes) > 0:
            ...
            while DEFAULT_IMAGE_TOKEN in text:
                num_tokens = self._get_visual_seq_len(grid_sizes[image_idx])
                text = text.replace(
                    DEFAULT_IMAGE_TOKEN, '<placeholder>' * num_tokens, 1
                )
                image_idx += 1
        ...
        **kwargs: Unpack[Videollama3Qwen2ProcessorKwargs],
        ...
        if conversation is not None:
            ...
            return self._process_conversation(
                conversation, images, return_labels, **kwargs
            )
    ...
    def apply_chat_template(
        ...
        image_token: str | None = DEFAULT_IMAGE_TOKEN,
        **kwargs,
    ) -> str:
        """
